...
function tilFil ($prosessID, $vSize, $wSet) {
    ...
    Write-Output "Total størelse an Working Set i minne: $wSet KB" >> $navn
...
foreach ($id in $args) {
    ...
    $prosessID = $(Get-Process -id $id) 2>$null
                    # Hvis PID existerer, hent relevant data og skriv det til fil
    if($prosessID) {
                                      
        tilFil $id $(($prosessID ).VirtualMemorySize ) $(($prosessID).WorkingSet)

    }         
    else {     
                    # Hvis PID ikke exsisterer, print feilmelding til consol
